...
no_review <- con1 %>% dplyr::select(External_ID, Species) %>% 
	group_by(External_ID) %>%
	mutate(Index = row_number()) %>%
	ungroup() %>%
	pivot_wider(names_from = Index, 
...
colnames(batch3) <- c("External_ID", "Species")

id <- paste0(batch3$External_ID, batch3$Species) 
con1$ID <- paste0(con1$External_ID, con1$Species)
