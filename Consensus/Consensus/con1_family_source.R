...
no_review <- con1 %>% dplyr::select(External_ID, Family) %>% 
	group_by(External_ID) %>%
	mutate(Index = row_number()) %>%
	ungroup() %>%
	pivot_wider(names_from = Index, 
...
colnames(batch3) <- c("External_ID", "Family")

id <- paste0(batch3$External_ID, batch3$Family) 
con1$ID <- paste0(con1$External_ID, con1$Family)
